...
$backup = "$schemaPath.timeoff-s2-fixed-backup"
...
model TimeOffRequest {
  ...
  timeOffImport TimeOffImport @relation(fields: [timeOffImportId], references: [id], onDelete: Cascade)
...
$newSchema = $schema.TrimEnd() + "`r`n`r`n" + $addition.Trim() + "`r`n"
...
Write-Host "Time Off S2 schema added successfully."
